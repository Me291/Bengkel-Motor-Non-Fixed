...
if (isset($_GET['id'])) {
  $id = $_GET['id'];
  $delete = mysqli_query($conn,"DELETE FROM `daftar_service` WHERE `id` = '$id'");
  echo "<script>alert('Data Berhasil Dihapus');</script>";
	echo "<meta http-equiv='refresh' content='1;url=Data Service Konsumen.php'>";
}
...
<head>
  <meta charset="UTF-8" />
  <title>Data Service Konsumen</title>
    ...
    <section class="main">
      ...
      <section class="attendance">
        <div class="attendance-list">
          <h1>Informasi Data Service Konsumen</h1>
          <table class="table">
            <thead>
              <tr>
                <th>No</th>
            ...
            <tbody>
              <?php $nomor=1;?>
              <?php $ambil = $conn->query("SELECT * FROM daftar_service");?>
              <?php while($pecah = $ambil->fetch_assoc()){?>
              <tr>
                <td><?php echo $nomor;?></td>
                <td><?php echo $pecah['nama'];?></td>
                <td><?php echo $pecah['email'];?></td>
                <td><?php echo $pecah['nomor_kendaraan'];?></td>
                <td><?php echo $pecah['no_telpon'];?></td>
                <td><?php echo $pecah['jenis_kendaraan'];?></td>
                <td>
                  <a href="Data Service Konsumen.php?halaman=hapus Data Service Konsumen&id=<?php echo $pecah['id']?>" style ="width:auto;" class="btn btn-primary"><i class="fa-sharp fa-solid fa-trash"></i></a>
                </td>
              </tr>
              <?php $nomor++; ?>
              <?php } ?>
          ...
          <a href="Data Service Homes.php" class="btn btn-primary">Data Service Homes</a>
        </div>
      </section>
    </section>
  </div>
